...
use nebula_terminal::term::{LineDamageBounds, TermDamageIterator};
...
impl FrameDamage {
    ...
    pub fn damage_line(&mut self, damage: LineDamageBounds) {
        // A line index can briefly outrun this frame's buffer while a resize
        // propagates from the terminal to the damage tracker (off-by-one for a
        // frame or two). Such a line is off the tracked viewport and Nebula
        // repaints the whole window every frame, so skipping it is safe.
...
/// Iterator which converts `nebula_terminal` damage information into renderer damaged rects.
