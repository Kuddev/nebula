...
impl Display {
    ...
    pub fn nebula_commit_line(&mut self, state: &mut NebulaPaneState) {
        ...
        // the tab icon has to be captured here.
        state.last_committed = line;
        Self::nebula_clear_line(state);
    }
    ...
    #[cfg(windows)]
    fn nebula_input_from_raw_grid<T: EventListener>(
        terminal: &Term<T>,
        cursor: Point,
    ) -> Option<String> {
        let grid = terminal.grid();
        ...
        // Find the last prompt arrow at or before the cursor. This deliberately
        // reads the raw terminal grid, not RenderableCell: renderables omit
        ...
        let mut input = String::with_capacity(cursor_col.saturating_sub(arrow_col + 1));
        for col in (arrow_col + 1)..cursor_col {
            let cell: &Cell = &grid[cursor.line][Column(col)];
            if cell.flags.intersects(Flags::WIDE_CHAR_SPACER | Flags::LEADING_WIDE_CHAR_SPACER) {
                continue;
            }
    ...
    fn draw_chrome(&mut self) {
        ...
        if !self.nebula_sidebar_collapsed && !self.nebula_settings_open {
            ...
            for (index, (tab_x, row_y, tab_w, tab_h)) in tab_layout.tabs.iter().copied().enumerate()
            {
                ...
                let label = self.nebula_tab_labels.get(index).map(String::as_str).unwrap_or(".");
