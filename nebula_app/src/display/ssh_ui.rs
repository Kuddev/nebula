...
/// Tab 环上的一个停靠点。顺序只在 [`SshHostEditor::slots`] 一处生成。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SshEditorSlot {
    Field(SshEditorField),
    Test,
    Cancel,
    Save,
...
impl SshHostEditor {
    ...
    /// 渲染路径 panic。
    pub fn slot_of(&self, target: SshEditorSlot) -> usize {
        self.slots().iter().position(|slot| *slot == target).unwrap_or(0)
    }
