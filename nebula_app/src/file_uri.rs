...
//!
//! Only compiled on Windows — every other platform keeps the upstream
...
fn file_uri_to_local_path_with(
    ...
    if host_is_local(host) {
        ...
        // Remote host → UNC share (`\\HOST\path`). We keep the behaviour the
        // upstream `cmd start` link action already had for these; we just build
        // a path `explorer` understands instead of a URI.
