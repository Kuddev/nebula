...
impl Gles2Renderer {
    /// Forget the cached glyph-atlas binding. MUST be called after anything
    /// outside this renderer binds `TEXTURE_2D` (image quads, logo icons):
    /// batch rendering skips `glBindTexture` when `active_tex` already equals
    /// the batch atlas, so a stale cache makes every glyph sample a foreign
    /// texture — text silently vanishes.
    pub fn invalidate_active_tex(&mut self) {
        self.active_tex = 0;
    }
