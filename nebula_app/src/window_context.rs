...
impl WindowContext {
    ...
    fn sync_chrome_tabs(&mut self) {
        ...
        for tab in &self.tabs {
            ...
            let mut label = pane.map(Self::chrome_tab_label).unwrap_or_default();
    ...
    pub fn handle_event(
        ...
        // Resolve which pane this batch targets: a UserEvent names its source
        // pane; otherwise it's the focused pane of the active tab.
        let target_pane_id = self.event_queue.iter().find_map(|event| match event {
            WinitEvent::UserEvent(event) => event.terminal_tab_id(),
            _ => None,
        });
        let focused_id = target_pane_id.unwrap_or_else(|| self.focused_pane_id());
