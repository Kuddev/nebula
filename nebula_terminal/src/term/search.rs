...
mod tests {
    ...
    fn regex_right() {
        ...
        let mut regex = RegexSearch::new("Neb.*123").unwrap();
    ...
    fn nested_regex() {
        #[rustfmt::skip]
        let term = mock_term("\
            Neb -> Nebulatty -> ulatty\r\n\
            ulatty\
        ");

        // Greedy stopped at linebreak.
        let mut regex = RegexSearch::new("Neb.*ulatty").unwrap();
        ...
        let mut regex = RegexSearch::new("Neb[^y]*ulatty").unwrap();
        let start = Point::new(Line(0), Column(0));
    ...
    #[test]
    fn skip_dead_cell() {
        let term = mock_term("nebulatty");

        // Make sure dead state cell is skipped when reversing.
        let mut regex = RegexSearch::new("nebulat").unwrap();
        let start = Point::new(Line(0), Column(0));
