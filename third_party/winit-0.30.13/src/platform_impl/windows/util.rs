...
/// Windows 10 build number used to gate the legacy mixed-DPI reposition fix.
///
/// Windows 11 changed the behavior of `WM_DPICHANGED` so the old workaround
/// can move a dragged window back to the monitor seam. Keep the version query
/// lazy: it is read only when a DPI transition is actually being handled.
pub(crate) static WIN10_BUILD_VERSION: LazyLock<Option<u32>> = LazyLock::new(|| {
    type RtlGetVersion = unsafe extern "system" fn(*mut OSVERSIONINFOW) -> NTSTATUS;
    let handle = get_function!("ntdll.dll", RtlGetVersion);

    let Some(rtl_get_version) = handle else {
        return None;
    };
    ...
    unsafe {
        ...
        let status = rtl_get_version(&mut version);
        (status >= 0 && version.dwMajorVersion == 10 && version.dwMinorVersion == 0)
