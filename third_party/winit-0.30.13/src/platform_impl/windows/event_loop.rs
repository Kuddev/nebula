...
unsafe fn public_window_callback_inner(
    ...
    let callback = || match msg {
        ...
        WM_DPICHANGED => {
            ...
            let new_outer_rect = if util::WIN10_BUILD_VERSION
                .is_some_and(|version| version < 22000)
            {
                apply_win10_dpi_adjustment(
                    ...
                    margin_left,
                    margin_top,
                    new_physical_inner_size,
                    window_flags,
                    dragging_window,
                )
            } else {
                // Windows 11 already supplies a usable mixed-DPI target rect.
                // Reapplying the Windows 10 nudge here pushes a dragged window
...
/// Apply the legacy Windows 10 mixed-DPI reposition workaround.
///
/// Windows 11 already supplies a usable `WM_DPICHANGED` target rectangle. The
/// workaround is kept isolated here so the caller can gate it by the OS build
/// without changing the event ordering or the `EventLoop<T>` API.
fn apply_win10_dpi_adjustment(
    window: HWND,
    suggested_rect: RECT,
    margin_left: i32,
    margin_top: i32,
    new_physical_inner_size: PhysicalSize<u32>,
    ...
    // Preserve the cursor's relative position in the title bar during a live drag.
    if dragging_window {
        let bias = {
            let cursor_pos = {
    ...
    // On affected Windows 10 builds, nudge the calculated rectangle to the
    // monitor that owns the current DPI. Windows 11 must skip this block.
    ...
    let wrong_monitor_rect = get_monitor_rect(conservative_rect_monitor);
