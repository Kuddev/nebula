...
- Fixed a crash being indistinguishable from a clean exit; the restored window now says so in a transient success toast.
- Fixed the crashed session being overwritten after three failed launches — it is now kept as `session.crashed.json`.
- Recovery notices now use the right layer: completed recovery uses a short success toast, while the crash-loop breaker stays in the message bar with the quarantined session path.
- Fixed the message-bar close button disappearing when a notice contains CJK text; the button is now a reusable UI widget with visible hover feedback.
...
- 新增静默标签页右侧的 shell 短标：`pwsh`、`cmd`、`bash`、WSL 发行版名，SSH 标签显示 `ssh`。
...
- 修复崩溃与正常退出分不清的问题，恢复后会用自动消失的 success toast 说明上次是异常退出。
- 修复连续三次启动失败后出问题的会话被覆盖的问题，现在保留为 `session.crashed.json`。
- 恢复提示按待办动作分层：已完成的恢复成功提示走短暂 toast，断路器仍留在消息栏并保留隔离会话路径。
- 修复中文消息栏关闭按钮消失的问题；按钮改为可复用的 UI 组件并提供可见的悬停反馈。
...
- `NebulaTerminal-v0.9.0-windows-x64.zip`: `2D178C07EE3AB556FF64772E9B84F2FC5CA85F5EF80FDF1F64F2189E7EE3C783`
- `NebulaTerminal-0.9.0-windows-x64-setup.exe`: `11A6A547D272C02674C5A280AA053F2365CAB5F2BEF44718B6FBBCAF57B1311A`
