...
- Fixed finished AI commands remaining stuck in the running state — Process-tree evidence now disproves stale shell state after Codex or Claude returns to an idle prompt. Addresses [#42](https://github.com/Kuddev/nebula/issues/42) and [#54](https://github.com/Kuddev/nebula/issues/54).
- Fixed GPUI windows showing the default white placeholder in taskbar previews and Alt+Tab by setting both native Windows icon sizes. Addresses [#51](https://github.com/Kuddev/nebula/issues/51).
...
- 修复 AI 命令结束后仍错误保持“运行中” — Codex 或 Claude 回到空闲提示符后，进程树证据会推翻过期的 shell 状态。对应 [#42](https://github.com/Kuddev/nebula/issues/42) 与 [#54](https://github.com/Kuddev/nebula/issues/54)。
- 修复 GPUI 窗口在任务栏悬停预览与 Alt+Tab 中显示默认白框占位图；窗口现在会设置两种原生 Windows 图标尺寸。对应 [#51](https://github.com/Kuddev/nebula/issues/51)。
