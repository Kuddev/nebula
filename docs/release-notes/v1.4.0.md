...
- Fixed new Windows panes inheriting stale environment variables from the long-running Nebula process; new panes now rebuild the current machine and user environment. Addresses [#68](https://github.com/Kuddev/nebula/issues/68).
...
- Fixed Settings crashing when proxy fields contain non-ASCII text or full-width punctuation. Addresses [#67](https://github.com/Kuddev/nebula/issues/67).
- Fixed File Tree, Git, top-tab, and sidebar controls leaking clicks into title-bar drag or parent collapse handlers. Addresses [#62](https://github.com/Kuddev/nebula/issues/62).
- Fixed GPUI diagnostics and Windows panic reporting causing a second panic while reporting the first failure. Addresses [#70](https://github.com/Kuddev/nebula/issues/70).
...
- Improved terminal TeX rendering with content-based detection and compact GPUI projection — Formulas work through WSL, SSH, and full-screen Agent interfaces while selection, cursor, completion, IME, links, and terminal source coordinates remain correct. Includes work contributed by [@liu-ws](https://github.com/liu-ws) in [#55](https://github.com/Kuddev/nebula/pull/55).
...
- 新增版本化 Runtime 控制 API 与资源式 CLI — `nebula env`、`window`、`tab`、`pane`、`agent` 通过可发现的 JSON 操作统一控制 Nebula，并同步更新 Bash、Fish 与 Zsh 补全。
...
- 新增 GPUI 设置页内置 English 与简体中文语言包 — 语言可跟随系统或显式选择，跨启动保留，并只在内存中切换，无需运行时读取翻译文件。此项仅对应 [#71](https://github.com/Kuddev/nebula/issues/71) 的 GPUI 设置页范围；其他尚未翻译的界面不在本次发布范围内。
...
- 修复 Windows 新建 pane 继承 Nebula 常驻进程中的过期环境变量；新 pane 现在会重建当前机器与用户环境。对应 [#68](https://github.com/Kuddev/nebula/issues/68)。
- 修复快速终端全局快捷键、首次打开空白条、窗口尺寸错误，以及混入普通工作区生命周期的问题。
- 修复代理输入框包含非 ASCII 文本或全角标点时设置页崩溃。对应 [#67](https://github.com/Kuddev/nebula/issues/67)。
- 修复文件树、Git、顶部标签与侧栏控件把点击泄漏给标题栏拖动或父级折叠处理。对应 [#62](https://github.com/Kuddev/nebula/issues/62)。
- 修复 GPUI 诊断与 Windows panic 报告在记录原始故障时触发第二次 panic。对应 [#70](https://github.com/Kuddev/nebula/issues/70)。
...
- 改进终端 TeX 渲染，采用基于内容的识别与 GPUI 紧凑投影 — WSL、SSH 和全屏 Agent 界面中的公式可以正常渲染，同时保持选区、光标、补全、输入法、链接与终端源坐标准确。包含 [@liu-ws](https://github.com/liu-ws) 在 [#55](https://github.com/Kuddev/nebula/pull/55) 中贡献的工作。
...
<a href="https://github.com/liu-ws"><img src="https://github.com/liu-ws.png?size=96" width="64" height="64" alt="@liu-ws avatar"></a>
